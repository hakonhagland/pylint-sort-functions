...
class TestFunctionSortChecker(CheckerTestCase):
    ...
    def test_visit_module_calls_utils(self) -> None:
        ...
        with patch('pylint_sort_functions.utils.get_functions_from_node') as mock_get_functions, \
             patch('pylint_sort_functions.utils.are_functions_sorted') as mock_are_sorted:
    ...
    def test_visit_classdef_calls_utils(self) -> None:
        ...
        with patch('pylint_sort_functions.utils.get_methods_from_class') as mock_get_methods, \
             patch('pylint_sort_functions.utils.are_methods_sorted') as mock_are_sorted, \
             patch('pylint_sort_functions.utils.are_functions_properly_separated') as mock_are_separated:
            ...
            expected_calls = [
                ...
                (("mixed-function-visibility",), {"node": mock_node, "args": ("class TestClass",)}),
            ]
            assert self.checker.add_message.call_count == 2
            for expected_call in expected_calls:
                assert expected_call in [
                    (call.args, call.kwargs) for call in self.checker.add_message.call_args_list
                ]
