...
class TestIssue25SyntaxCorruption:
    ...
    def test_complex_class_auto_sort_preserves_syntax(self) -> None:
        ...
        This is the main reproduction case from issue #25. The test creates a Python file
        with multiple dialog classes (similar to PyQt applications) that have:
        ...
        original_content = dedent("""
            '''Complex file with multiple dialog classes that triggers auto-sort corruption.'''
        ...
        try:
            ...
            # Additional validations to ensure proper structure
            ...
            for line_content in super_calls:
                ...
                for i in range(line_index - 1, -1, -1):
                    ...
                    if lines[i].strip().startswith("def ") and not lines[i].startswith(
                        ...
                        # Found a module-level function before a class - super() is orphaned
                        break

                assert found_class, (
                    f"Found orphaned super() call at line {line_index + 1}: {line_content.strip()}\n"
            ...
            for method_line in method_definitions:
                assert method_line.startswith("    "), (
                    f"Found method definition not properly indented: {method_line.strip()}\n"
            ...
            # Methods like get_result appear in multiple classes - this is expected and OK
            # But duplicate definitions in the same context would be a corruption

            # The key validation: if sorting was applied, methods within each class should be sorted
            if was_modified:
                ...
                for line in lines:
                    ...
                    if stripped.startswith("class "):
                        ...
                        if current_class and current_class_methods:
                            # Check if methods are sorted (public first, then private, each alphabetically)
                            public_methods = [
                                m
                                for m in current_class_methods
                            ...
                            assert public_methods == sorted(public_methods), (
                                f"Public methods in class {current_class} are not sorted: {public_methods}"
                            )

                            # Private methods should be alphabetically sorted
                            assert private_methods == sorted(private_methods), (
                                f"Private methods in class {current_class} are not sorted: {private_methods}"
                            )
    ...
    def test_class_with_complex_multiline_methods_preserves_syntax(self) -> None:
        ...
        signatures without creating syntax errors. The original test was incorrectly
        flagging valid Python syntax as corruption.
        """
        original_content = dedent("""
            class TestClass:
        ...
        try:
            ...
            # Validate that methods are properly sorted (if sorting was needed)
            if was_modified:
                lines = result_content.splitlines()
                ...
                for line in lines:
                    ...
                    if stripped.startswith("class "):
                        ...
                        assert line.startswith("    "), (
                            f"Method not properly indented: {line}"
                        )
                        method_name = stripped.split("(")[0].replace("def ", "")
                        method_lines.append(method_name)

                # Methods should be sorted: another_method before method_with_complex_args
                expected_order = ["another_method", "method_with_complex_args"]
                assert method_lines == expected_order, (
                    f"Methods not properly sorted. Expected {expected_order}, got {method_lines}"
                )
    ...
    def test_preserves_complex_inheritance_with_super_calls(self) -> None:
        ...
        try:
            ...
            # Find all super() calls and verify they're properly indented (in class methods)
            super_calls = [
                (i, line) for i, line in enumerate(lines) if "super()" in line
            ]

            for line_num, line_content in super_calls:
                # super() calls should be indented (inside methods, which are inside classes)
                assert line_content.startswith("        "), (
                    f"super() call at line {line_num + 1} is not properly indented: {line_content}\n"
                ...
                assert found_method, (
                    f"super() call at line {line_num + 1} has no method context: {line_content.strip()}"
                )
    ...
    def test_syntax_validation_rollback_coverage(self) -> None:
        ...
        try:
            ...
            # Test the case where SyntaxError has lineno but no text
            with patch.object(sorter, "_sort_functions_in_content") as mock_sort:
                # Create a SyntaxError with lineno but no text attribute
                def mock_compile(source: str, filename: str, mode: str) -> object:
                    if "invalid" in source:
                        error = SyntaxError("invalid syntax")
                        error.lineno = 1
                        error.text = None  # This will trigger the else branch
                        raise error
                    return compile(source, filename, mode)

                mock_sort.return_value = "invalid python syntax with no text"

                with patch("builtins.compile", side_effect=mock_compile):
                    # This should trigger the SyntaxError path with no text
                    was_modified = sorter.sort_file(temp_path)
                    assert not was_modified
